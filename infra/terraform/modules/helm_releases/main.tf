...
# Schema DDL + Grafana views live in clickhouse_schema.sql (rendered with the
# database name) so they're easy to read/edit outside the .tf file.
resource "kubernetes_config_map" "clickhouse_schema" {
  ...
  data = {
    "schema.sql" = templatefile("${path.module}/clickhouse_schema.sql", {
      database = var.clickhouse_db
    })
  }
}
...
resource "kubernetes_job" "clickhouse_init" {
  ...
  spec {
    ...
    template {
      metadata {
        # Re-run the Job whenever schema.sql changes (the pod template is
        # immutable, so a new hash forces Terraform to replace the Job).
        annotations = {
          "schema-sha" = substr(sha256(kubernetes_config_map.clickhouse_schema.data["schema.sql"]), 0, 12)
        }
      }
      spec {
        ...
        volume {
          name = "schema"
          config_map {
            name = kubernetes_config_map.clickhouse_schema.metadata[0].name
          }
        }
      }
    }
...
resource "helm_release" "grafana" {
  ...
  values = [yamlencode({
    ...
    # Sidecar auto-loads dashboards from ConfigMaps labeled grafana_dashboard=1.
    sidecar = {
      dashboards = {
        enabled = true
...
# Realtime logistics dashboard, picked up by the Grafana sidecar via the
# grafana_dashboard label. Edit grafana_dashboard.json to change panels.
resource "kubernetes_config_map" "grafana_dashboard_logistics" {
  metadata {
    name      = "grafana-dashboard-logistics"
    namespace = "monitoring"
    labels    = { grafana_dashboard = "1" }
  }
  data = {
    "logistics.json" = file("${path.module}/grafana_dashboard.json")
  }
  depends_on = [kubernetes_namespace.namespaces]
}
